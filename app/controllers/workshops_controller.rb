class WorkshopsController < ApplicationController
  ...
  def rsvp
    ...
    if role_params.nil?
      @invitation = SessionInvitation.where(sessions: @workshop, member: current_user, attending: true).first
    else
      @invitation = SessionInvitation.where(sessions: @workshop, member: current_user, role: role_params).first_or_create
    end
