$(function() {
  var handler = StripeCheckout.configure({
    ...
    token: function(token) {
      ...
      window.executeRecaptchaForDonateAsync().then((recaptchaToken) => {
        $.ajax({
          type: "POST",
          url: "/donations",
          data: {
            ...
            name: name,
            data: token,
            'g-recaptcha-response[donate]': recaptchaToken
          }
        })
        ...
        .fail(function(xhr, status, e) {
          $(".message").html(
            "Your transaction has not been succesful. Please try again."
          );
        });
      });
    }
